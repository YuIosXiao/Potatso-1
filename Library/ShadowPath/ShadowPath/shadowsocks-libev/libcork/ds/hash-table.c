                              ...
                              size_t desired_count)
{
    ...
    CORKDEBUG("Allocate %zu bins", table->bin_count);
...
cork_hash_table_clear(struct cork_hash_table *table)
{
    ...
    CORKDEBUG("(clear) Remove all entries");
        ...
        if (old_bins != NULL) {
            ...
            for (i = 0; i < old_bin_count; i++) {
                ...
                while (!cork_dllist_is_end(bin, curr)) {
                    ...
                    CORKDEBUG("      Rehash %p from bin %zu to bin %zu",
                          entry, i, bin_index);
...
{
    CORKDEBUG("    Reached maximum density; rehash");
                               ...
                               cork_hash hash, const void *key)
{
    ...
    if (table->bin_count == 0) {
        CORKDEBUG("(get) Empty table when searching for key %p "
              "(hash 0x%08" PRIx32 ")",
              key, hash);
        return NULL;
    }
    ...
    CORKDEBUG("(get) Search for key %p (hash 0x%08" PRIx32 ", bin %zu)",
          key, hash, bin_index);
                         ...
                         cork_hash hash, const void *key)
{
    ...
    if (entry == NULL) {
        ...
        CORKDEBUG("  Extract value pointer %p", entry->value);
        return entry->value;
    }
}
                                   ...
                                   cork_hash hash, void *key, bool *is_new)
{
    ...
    if (table->bin_count > 0) {
        ...
        bin_index = bin_index(table, hash);
        CORKDEBUG("(get_or_create) Search for key %p "
        ...
        while (!cork_dllist_is_end(bin, curr)) {
            ...
            if (table->equals(table->user_data, key, entry->public.key)) {
                CORKDEBUG("    Match");
                CORKDEBUG("    Return value pointer %p", entry->public.value);
                *is_new = false;
                return &entry->public;
            }
        ...
        CORKDEBUG("(get_or_create) Search for key %p (hash 0x%08" PRIx32 ")",
              key, hash);
        CORKDEBUG("  Empty table");
        cork_hash_table_rehash(table);
        bin_index = bin_index(table, hash);
    }

    CORKDEBUG("    Allocate new entry");
    entry = cork_hash_table_new_entry(table, hash, key, NULL);
    CORKDEBUG("    Created new entry %p", entry);

    CORKDEBUG("    Add entry into bin %zu", bin_index);
                         ...
                         bool *is_new, void **old_key, void **old_value)
{
    ...
    if (table->bin_count > 0) {
        ...
        bin_index = bin_index(table, hash);
        CORKDEBUG("(put) Search for key %p (hash 0x%08" PRIx32 ", bin %zu)",
              key, hash, bin_index);
        ...
        while (!cork_dllist_is_end(bin, curr)) {
            ...
            CORKDEBUG("  Check entry %p", entry);
            if (table->equals(table->user_data, key, entry->public.key)) {
                CORKDEBUG("    Found existing entry; overwriting");
                CORKDEBUG("    Return old key %p", entry->public.key);
                if (old_key != NULL) {
                    *old_key = entry->public.key;
                }
                CORKDEBUG("    Return old value %p", entry->public.value);
                if (old_value != NULL) {
                    *old_value = entry->public.value;
                }
                CORKDEBUG("    Copy key %p into entry", key);
                entry->public.key = key;
                CORKDEBUG("    Copy value %p into entry", value);
                entry->public.value = value;
                if (is_new != NULL) {
                    *is_new = false;
                }
        ...
        CORKDEBUG("(put) Search for key %p (hash 0x%08" PRIx32 ")",
              key, hash);
        CORKDEBUG("  Empty table");
        cork_hash_table_rehash(table);
        bin_index = bin_index(table, hash);
                            ...
                            void **deleted_key, void **deleted_value)
{
    ...
    if (table->bin_count == 0) {
        CORKDEBUG("(delete) Empty table when searching for key %p "
              "(hash 0x%08" PRIx32 ")",
              key, hash);
        return false;
    }
    ...
    bin_index = bin_index(table, hash);
    CORKDEBUG("(delete) Search for key %p (hash 0x%08" PRIx32 ", bin %zu)",
          key, hash, bin_index);
    ...
    while (!cork_dllist_is_end(bin, curr)) {
        ...
        if (table->equals(table->user_data, key, entry->public.key)) {
            ...
            CORKDEBUG("    Remove entry from hash bin %zu", bin_index);
            cork_dllist_remove(curr);
            table->entry_count--;

            CORKDEBUG("    Free entry %p", entry);
            cork_hash_table_free_entry(table, entry);
            return true;
        }
...
{
    struct cork_dllist_item  *curr;
    CORKDEBUG("Map across hash table");
...
{
    CORKDEBUG("Iterate through hash table");
...
cork_hash_table_iterator_next(struct cork_hash_table_iterator *iterator)
{
    ...
    entry = cork_container_of
        (curr, struct cork_hash_table_entry_priv, insertion_order);
    CORKDEBUG("    Return entry %p", entry);
