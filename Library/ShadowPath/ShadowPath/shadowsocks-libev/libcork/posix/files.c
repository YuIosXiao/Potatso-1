...
#if CORK_DEBUG_FILES
#include <stdio.h>
#define POSIXDEBUG(...) fprintf(stderr, __VA_ARGS__)
#else
#define POSIXDEBUG(...) /* no debug messages */
#endif
                    ...
                    unsigned int flags)
{
    ...
    if (file->type == CORK_FILE_DIRECTORY) {
        ...
        POSIXDEBUG("  Exists and not a directory!\n");
        cork_system_error_set_explicit(EEXIST);
        return -1;
    }
