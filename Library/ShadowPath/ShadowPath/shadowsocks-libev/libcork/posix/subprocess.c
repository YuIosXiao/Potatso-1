...
#include <stdio.h>
#define SSDEBUG(...) fprintf(stderr, __VA_ARGS__)
#else
#define SSDEBUG(...) /* no debug messages */
#endif
...
cork_read_pipe_open(struct cork_read_pipe *p)
{
    if (p->consumer != NULL) {
        ...
        SSDEBUG("[read] Opening pipe\n");
        rii_check_posix(pipe(p->fds));
        SSDEBUG("[read]   Got read=%d write=%d\n", p->fds[0], p->fds[1]);
        SSDEBUG("[read]   Setting non-blocking flag on read pipe\n");
...
{
    if (p->fds[0] != -1) {
        SSDEBUG("[write] Closing read pipe %d\n", p->fds[0]);
        rii_check_posix(close(p->fds[0]));
        p->fds[0] = -1;
    }
    return 0;
...
{
    if (p->fds[1] != -1) {
        SSDEBUG("[write] Closing write pipe %d\n", p->fds[1]);
        rii_check_posix(close(p->fds[1]));
        p->fds[1] = -1;
    }
    return 0;
...
{
    SSDEBUG("[write] Opening writer pipe\n");
    rii_check_posix(pipe(p->fds));
    SSDEBUG("[write]   Got read=%d write=%d\n", p->fds[0], p->fds[1]);
    return 0;
}
...
cork_subprocess_abort(struct cork_subprocess *self)
{
    if (self->pid > 0) {
        ...
        SSDEBUG("Terminating child process %d\n", (int) self->pid);
...
{
    size_t  i;
    SSDEBUG("Starting subprocess group\n");
...
{
    SSDEBUG("Aborting subprocess group\n");
...
cork_subprocess_group_wait(struct cork_subprocess_group *group)
{
    unsigned int  spin_count = 0;
    bool  progress;
    SSDEBUG("Waiting for subprocess group to finish\n");
