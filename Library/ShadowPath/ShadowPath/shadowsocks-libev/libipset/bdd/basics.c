...
ipset_node_incref(struct ipset_node_cache *cache, ipset_node_id node_id)
{
    if (ipset_node_get_type(node_id) == IPSET_NONTERMINAL_NODE) {
        ...
        IPDEBUG("        [incref " IPSET_NODE_ID_FORMAT "]",
...
ipset_node_decref(struct ipset_node_cache *cache, ipset_node_id node_id)
{
    if (ipset_node_get_type(node_id) == IPSET_NONTERMINAL_NODE) {
        struct ipset_node  *node =
            ipset_node_cache_get_nonterminal(cache, node_id);
        IPDEBUG("        [decref " IPSET_NODE_ID_FORMAT "]",
              IPSET_NODE_ID_VALUES(node_id));
        if (--node->refcount == 0) {
            IPDEBUG("        [free   " IPSET_NODE_ID_FORMAT "]",
                  IPSET_NODE_ID_VALUES(node_id));
                             ...
                             ipset_node_id low, ipset_node_id high)
{
     ...
     * in the cache. */
    IPDEBUG("        [search nonterminal(x%u? "
          IPSET_NODE_ID_FORMAT ": " IPSET_NODE_ID_FORMAT ")]",
          variable, IPSET_NODE_ID_VALUES(high), IPSET_NODE_ID_VALUES(low));
    ...
    if (!is_new) {
        ...
        IPDEBUG("        [reuse  " IPSET_NODE_ID_FORMAT "]",
              IPSET_NODE_ID_VALUES(node_id));
        ...
        IPDEBUG("        [new    " IPSET_NODE_ID_FORMAT "]",
                    ...
                    ipset_assignment_func assignment, const void *user_data)
{
    ...
    IPDEBUG("Evaluating BDD node " IPSET_NODE_ID_FORMAT,
          IPSET_NODE_ID_VALUES(node_id));
    ...
    while (ipset_node_get_type(curr_node_id) == IPSET_NONTERMINAL_NODE) {
        ...
        IPDEBUG("[%3u] Nonterminal " IPSET_NODE_ID_FORMAT,
    ...
    IPDEBUG("Evaluated result is %u", ipset_terminal_value(curr_node_id));
                ...
                ipset_value g, ipset_node_id h)
{
    ...
    if (f->current_var == f->var_count) {
        ...
        IPDEBUG("[%3u] F is terminal (value %u)", f->current_var, f->value);

        if (f->value == 0) {
            IPDEBUG("[%3u] 0? " IPSET_NODE_ID_FORMAT ": " IPSET_NODE_ID_FORMAT
                  " = " IPSET_NODE_ID_FORMAT,
                  f->current_var,
            ...
            IPDEBUG("[%3u] 1? " IPSET_NODE_ID_FORMAT ": " IPSET_NODE_ID_FORMAT
                  " = " IPSET_NODE_ID_FORMAT,
    ...
    if (h == ipset_terminal_node_id(g)) {
        IPDEBUG("[%3u] F? " IPSET_NODE_ID_FORMAT ": " IPSET_NODE_ID_FORMAT
              " = " IPSET_NODE_ID_FORMAT,
     ...
     * nonterminal. */
    IPDEBUG("[%3u] F is nonterminal", f->current_var);
    ...
    if (ipset_node_get_type(h) == IPSET_NONTERMINAL_NODE) {
        ...
        IPDEBUG("[%3u] H is nonterminal (variable %u)",
              f->current_var, h_node->variable);

        if (h_node->variable < f->current_var) {
            /* var(F) > var(H), so we only recurse down the H branches. */
            IPDEBUG("[%3u] Recursing only down H", f->current_var);
            IPDEBUG("[%3u]   Recursing high", f->current_var);
            result_high = ipset_apply_ite(cache, f, g, h_node->high);
            IPDEBUG("[%3u]   Back from high recursion", f->current_var);
            IPDEBUG("[%3u]   Recursing low", f->current_var);
            result_low = ipset_apply_ite(cache, f, g, h_node->low);
            IPDEBUG("[%3u]   Back from low recursion", f->current_var);
            ...
            IPDEBUG("[%3u] Recursing down both F and H", f->current_var);
            h_low = h_node->low;
            h_high = h_node->high;
        } else {
            /* var(F) < var(H), so we only recurse down the F branches. */
            IPDEBUG("[%3u] Recursing only down F", f->current_var);
            h_low = h;
            h_high = h;
        }
    } else {
        /* H in nonterminal, so we only recurse down the F branches. */
        IPDEBUG("[%3u] H is terminal (value %u)",
    ...
    if (f->assignment(f->user_data, f->current_var)) {
        ...
        IPDEBUG("[%3u]   x[%u] is set", f->current_var, f->current_var);
        IPDEBUG("[%3u]   Recursing high", f->current_var);
        f->current_var++;
        result_high = ipset_apply_ite(cache, f, g, h_high);
        f->current_var--;
        IPDEBUG("[%3u]   Back from high recursion: " IPSET_NODE_ID_FORMAT,
        ...
        IPDEBUG("[%3u]   Back from low recursion: " IPSET_NODE_ID_FORMAT,
        ...
        IPDEBUG("[%3u]   x[%u] is NOT set", f->current_var, f->current_var);
        IPDEBUG("[%3u]   Recursing high", f->current_var);
        fake_terminal_0.current_var = f->var_count;
        fake_terminal_0.var_count = f->var_count;
