...
{
    IPDEBUG("Stream contains v1 IP set");
    ...
    uint64_t  length;
    IPDEBUG("Reading encoded length");
    ...
    IPDEBUG("Length cap is %zu bytes.", cap);

    /* Read in the number of nonterminals. */

    uint32_t  nonterminal_count;
    IPDEBUG("Reading number of nonterminals");
    ...
    if (nonterminal_count == 0) {
        uint32_t  value;
        IPDEBUG("Reading single terminal value");
    ...
    for (i = 0; i < nonterminal_count; i++) {
        ...
        IPDEBUG("Read serialized node %d = (x%d? %" PRId32 ": %" PRId32 ")",
        ...
        if (low >= 0) {
            ...
            IPDEBUG("  Serialized ID %" PRId32 " is internal ID %u",
                  low, low_id);
        }
        ...
        IPDEBUG("Internal node %u = nonterminal(x%d? %u: %u)",
...
ipset_node_cache_load(FILE *stream, struct ipset_node_cache *cache)
{
    ...
    IPDEBUG("Reading IP set magic number");
    ...
    uint16_t  version;
    IPDEBUG("Reading IP set version");
