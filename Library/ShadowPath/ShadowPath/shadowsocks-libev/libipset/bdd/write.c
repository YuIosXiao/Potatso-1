    ...
    if (!is_new) {
        *dest = (intptr_t) entry->value;
        ...
        if (ipset_node_get_type(node_id) == IPSET_TERMINAL_NODE) {
            ...
            IPDEBUG("Writing terminal(%d)", value);
            ...
            *dest = value;
            ...
            IPDEBUG("Visiting node %u nonterminal(x%u? %u: %u)",
            ...
            IPDEBUG("Writing node %u as serialized node %d = (x%u? %d: %d)",
                  node_id, result,
         ...
         struct ipset_node_cache *cache, ipset_node_id root)
{
    ...
    IPDEBUG("Writing file footer");
    ei_check(save_data->write_footer(save_data, cache, root));

    IPDEBUG("Freeing file caches");
