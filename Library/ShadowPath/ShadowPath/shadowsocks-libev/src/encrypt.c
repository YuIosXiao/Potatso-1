...
get_cipher_type(int method)
{
    if (method <= TABLE || method >= CIPHER_NUM) {
        SSLOGE("get_cipher_type(): Illegal method");
        return NULL;
    }
    ...
    if (strcmp(polarname, CIPHER_UNSUPPORTED) == 0) {
        SSLOGE("Cipher %s currently is not supported by PolarSSL library",
             ciphername);
        return NULL;
    }
    ...
    if (strcmp(mbedtlsname, CIPHER_UNSUPPORTED) == 0) {
        SSLOGE("Cipher %s currently is not supported by mbed TLS library",
             ciphername);
        return NULL;
    }
...
{
    if (digest == NULL) {
        SSLOGE("get_digest_type(): Digest name is null");
        return NULL;
    }
...
{
    if (method <= TABLE || method >= CIPHER_NUM) {
        SSLOGE("cipher_context_init(): Illegal method");
        return;
    }
    ...
    if (cipher == NULL) {
        SSLOGE("Cipher %s not found in OpenSSL library", ciphername);
        FATAL("Cannot initialize cipher");
    }
    if (!EVP_CipherInit_ex(evp, cipher, NULL, NULL, NULL, enc)) {
        SSLOGE("Cannot initialize cipher %s", ciphername);
        exit(EXIT_FAILURE);
    }
    if (!EVP_CIPHER_CTX_set_key_length(evp, enc_key_len)) {
        EVP_CIPHER_CTX_cleanup(evp);
        SSLOGE("Invalid key length: %d", enc_key_len);
        exit(EXIT_FAILURE);
    }
    if (method > RC4_MD5) {
    ...
    if (cipher == NULL) {
        SSLOGE("Cipher %s not found in PolarSSL library", ciphername);
    ...
    if (cipher == NULL) {
        SSLOGE("Cipher %s not found in mbed TLS library", ciphername);
                      ...
                      int enc)
{
    ...
    if (iv == NULL) {
        SSLOGE("cipher_context_set_iv(): IV is null");
        return;
    }
    ...
    cipher_evp_t *evp = ctx->evp;
    if (evp == NULL) {
        SSLOGE("cipher_context_set_iv(): Cipher context is null");
        return;
    }
#if defined(USE_CRYPTO_OPENSSL)
...
{
    if (method <= TABLE || method >= CIPHER_NUM) {
        SSLOGE("enc_key_init(): Illegal method");
        return;
    }
...
#endif
            SSLOGE("Cipher %s not found in crypto library", supported_ciphers[method]);
            FATAL("Cannot initialize cipher");
        } while (0);
    }
...
enc_init(const char *pass, const char *method)
{
    ...
    if (method != NULL) {
        ...
        if (m >= CIPHER_NUM) {
            SSLOGE("Invalid cipher name: %s, use rc4-md5 instead", method);
            m = RC4_MD5;
        }
    }
    if (m == TABLE) {
