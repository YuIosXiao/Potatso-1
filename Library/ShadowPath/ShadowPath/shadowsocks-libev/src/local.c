    ...
    while (1) {
        ...
        if (server->stage == STAGE_STREAM) {
            ...
            if (!remote->direct) {
                ...
                if (err) {
                    SSLOGE("server invalid password or cipher");
                    close_and_free_remote(EV_A_ remote);
                    close_and_free_server(EV_A_ server);
                    return;
                }
            ...
            if (request->cmd == 3) {
                ...
                SSLOGE("unsupported cmd: %d", request->cmd);
            ...
            if (server->stage == STAGE_HANDSHAKE) {
                ...
                if (s < reply_size) {
                    SSLOGE("failed to send fake reply");
                    close_and_free_remote(EV_A_ remote);
                    close_and_free_server(EV_A_ server);
                    return;
                }
            ...
            if (atyp == 1) {
                ...
                bfree(abuf);
                SSLOGE("unsupported addrtype: %d", request->atyp);
                close_and_free_remote(EV_A_ remote);
                close_and_free_server(EV_A_ server);
                return;
            }
...
stat_update_cb()
{
    ...
    if (now - last > 1.0) {
        ...
        char stat[20];
        sprintf(stat, "%" PRIu64 "|%" PRIu64, tx, rx);
        
        write(stat_fd, stat, sizeof(stat));
        LOGI("Tunnel TX : %" PRIu64 "\n", tx);
        LOGI("Tunnel RX : %" PRIu64 "\n", rx);
        last = now;
    }
}
...
remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (!remote->direct) {
        ...
        int err = ss_decrypt(server->buf, server->d_ctx, BUF_SIZE);
            if (err) {
                SSLOGE("remote invalid password or cipher");
                close_and_free_remote(EV_A_ remote);
                close_and_free_server(EV_A_ server);
                return;
            }
