...
build_config(char *prefix, struct server *server)
{
    ...
    if (f == NULL) {
        if (verbose) {
            SSLOGE("unable to open config file");
        }
        ss_free(path);
        return;
    }
...
kill_server(char *prefix, char *pid_file)
{
    ...
    if (f == NULL) {
        if (verbose) {
            SSLOGE("unable to open pid file");
        }
        ss_free(path);
        return;
    }
...
manager_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (r > BUF_SIZE / 2) {
        SSLOGE("too large request: %d", (int)r);
        return;
    }
    ...
    if (strcmp(action, "add") == 0) {
        ...
        if (server == NULL || server->port[0] == 0 || server->password[0] == 0) {
            SSLOGE("invalid command: %s:%s", buf, get_data(buf, r));
            if (server != NULL) {
                ss_free(server);
            }
            goto ERROR_MSG;
