...
report_addr(int fd, int err_level)
{
    ...
    peer_name = get_peer_name(fd);
    if (peer_name != NULL) {
        SSLOGE("failed to handshake with %s", peer_name);
    ...
    if (fast_open) {
        ...
        if (s == -1) {
            if (errno == EPROTONOSUPPORT || errno == ENOPROTOOPT) {
                SSLOGE("fast open is not supported on this platform");
                fast_open = 0;
            } else {
                ERROR("setsockopt");
            }
...
create_and_bind(const char *host, const char *port, int mptcp)
{
    ...
    for (int i = 1; i < 8; i++) {
        ...
        if (s == 0) {
            ...
            sleep(pow(2, i));
            SSLOGE("failed to resolve server name, wait %.0f seconds", pow(2, i));
        }
    }

    if (s != 0) {
        SSLOGE("getaddrinfo: %s", gai_strerror(s));
        return -1;
    }
    ...
    if (server->stage == STAGE_STREAM) {
        ...
        if ((atyp & ADDRTYPE_MASK) == 1) {
            ...
            if (server->buf->len >= in_addr_len + 3) {
                ...
                SSLOGE("invalid header with addr type %d", atyp);
                report_addr(server->fd, MALFORMED);
                close_and_free_server(EV_A_ server);
                return;
            }
            ...
            if (name_len + 4 <= server->buf->len) {
                ...
                SSLOGE("invalid name length: %d", name_len);
                report_addr(server->fd, MALFORMED);
                close_and_free_server(EV_A_ server);
                return;
            }
...
server_resolve_cb(struct sockaddr *addr, void *data)
{
    ...
    if (addr == NULL) {
        SSLOGE("unable to resolve %s", query->hostname);
        close_and_free_server(EV_A_ server);
    } else {
        if (verbose) {
...
remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (err) {
        SSLOGE("invalid password or cipher");
        close_and_free_remote(EV_A_ remote);
        close_and_free_server(EV_A_ server);
        return;
    }
    ...
    if (peer_name != NULL) {
        ...
        if (acl) {
            if ((get_acl_mode() == BLACK_LIST && acl_match_host(peer_name) == 1)
                ...
                SSLOGE("Access denied from %s", peer_name);
                close(serverfd);
                return;
        ...
        if (!in_white_list && check_block_list(peer_name)) {
            SSLOGE("block all requests from %s", peer_name);
#ifdef __linux__
            set_linger(serverfd);
#endif
            close(serverfd);
...
main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "f:s:p:l:k:t:m:b:c:i:d:a:n:huUvA6",
        ...
        case '?':
            ...
            SSLOGE("Unrecognized option: %s", optarg);
            opterr = 1;
            break;
        }
    }
    ...
    while (server_num > 0) {
        ...
        // Setup UDP
        if (mode != TCP_ONLY) {
            const ss_addr_t EmptyStruct = {0};
            init_udprelay(server_host[index], server_port, NULL, 0, EmptyStruct, mtu, m,
    ...
    if (geteuid() == 0){
        ...
        SSLOGE("firewall setup requires running from root user");
        exit(-1);
    }
#endif
