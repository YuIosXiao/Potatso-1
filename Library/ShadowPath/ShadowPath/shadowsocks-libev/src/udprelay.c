                      ...
                      char *host, char *port, struct sockaddr_storage *storage)
{
    ...
    if (offset == 1) {
        SSLOGE("[udp] invalid header with addr type %d", atyp);
        return 0;
    }
...
create_server_socket(const char *host, const char *port)
{
    ...
    if (s != 0) {
        SSLOGE("[udp] getaddrinfo: %s", gai_strerror(s));
        return -1;
    }
...
remote_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (r == -1) {
        ...
        SSLOGE("[udp] remote_recv_recvfrom fragmentation");
        goto CLEAN_UP;
    }
    ...
    if (server_ctx->protocol_plugin) {
        ...
        if (protocol_plugin->client_udp_post_decrypt) {
            ...
            if ((int)buf->len < 0) {
                SSLOGE("client_udp_post_decrypt");
                close_and_free_remote(EV_A_ remote_ctx);
                return;
            }
            if ( buf->len == 0 )
    ...
    if (buf->len > packet_size) {
        SSLOGE("[udp] remote_recv_sendto fragmentation");
        goto CLEAN_UP;
    }
...
server_recv_cb(EV_P_ ev_io *w, int revents)
{
...
#if !defined(MODULE_TUNNEL) && !defined(MODULE_REDIR)
    if (frag) {
        SSLOGE("[udp] drop a message since frag is not 0, but %d", frag);
        goto CLEAN_UP;
    }
#endif
    ...
    if (buf->len > packet_size) {
        SSLOGE("[udp] server_recv_sendto fragmentation");
        goto CLEAN_UP;
    }
