...
run_as(const char *user)
{
    ...
    if (user[0]) {
        ...
        for (buflen = 128;; buflen *= 2) {
            ...
            if (err == 0 && pwd) {
                ...
                if (initgroups(pwd->pw_name, pwd->pw_gid) == -1) {
                    SSLOGE("Could not change supplementary groups for user '%s'.", pwd->pw_name);
                    return 0;
                }
                ...
                if (err) {
                    SSLOGE("run_as user '%s' could not be found: %s", user,
                            strerror(err));
                } else {
                    SSLOGE("run_as user '%s' could not be found.", user);
                }
                return 0;
        ...
        if (setgid(pwd->pw_gid) != 0) {
            SSLOGE("Could not change group id to that of run_as user '%s': %s",
                 pwd->pw_name, strerror(errno));
            return 0;
        }
        if (initgroups(pwd->pw_name, pwd->pw_gid) == -1) {
        ...
        if (setuid(pwd->pw_uid) != 0) {
            SSLOGE("Could not change user id to that of run_as user '%s': %s",
                 pwd->pw_name, strerror(errno));
            return 0;
        }
#endif
...
set_nofile(int nofile)
{
    ...
    if (setrlimit(RLIMIT_NOFILE, &limit) < 0) {
        if (errno == EPERM) {
            ...
            SSLOGE("invalid nofile, decrease nofile and try again");
            return -1;
        } else {
            SSLOGE("setrlimit failed: %s", strerror(errno));
            return -1;
        }
    }
